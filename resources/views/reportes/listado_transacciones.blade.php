...
<form method="GET" action="{{route("imprimir.transacciones")}}" target="_blank">
                @csrf
<div class="card-body">
...
<select required name="id_forma_pago" class="form-control select2" style="width: 100%;">
 <option value="">Seleccione</option>
               @foreach($forma_pagos as $forma_pago)
                  <option value="{{$forma_pago->id_forma_pago}}">  {{$forma_pago->descripcion}}</option>
              @endforeach
</select>
</div>
</div>
